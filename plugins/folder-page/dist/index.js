...
function mostRecentDates(entries) {
  let dates;
...
function normalizeIncludeTags(includeField) {
  if (includeField === null || includeField === void 0) return [];
  const values = Array.isArray(includeField) ? includeField : [includeField];
  const tags = values.flatMap(
    (value) => value === null || value === void 0 ? [] : String(value).split(",")
  ).map((value) => value.trim()).filter((value) => value.length > 0).map(slugTag);
  return [...new Set(tags)];
}
function normalizedPageTags(page) {
  const tags = page.frontmatter?.tags;
  if (!Array.isArray(tags)) return [];
  return tags.filter((tag) => typeof tag === "string").map(slugTag);
}
function mergeIncludedPages(pagesInFolder, allFiles, currentSlug, includeField) {
  ...
  const seenSlugs = new Set(
    pagesInFolder.map((page) => page.slug).filter((slug) => typeof slug === "string")
  );
...
var OttonFolderContent = ((opts) => {
  ...
  const FolderContent = (props) => {
    ...
    const folderPages = folder ? pagesFromTrie(folder, showSubfolders) : pagesFromAllFiles(allFiles, slug, showSubfolders);
    ...
    const cssClasses = Array.isArray(fileData.frontmatter?.cssclasses) ? fileData.frontmatter.cssclasses : [];
    const root = tree;
    const content = root.children.length === 0 ? fileData.description : htmlToJsx2(root);
    return /* @__PURE__ */ h2("div", { class: "popover-hint" }, /* @__PURE__ */ h2("article", { class: cssClasses.join(" ") }, /* @__PURE__ */ h2("div", { class: "markdown-preview-view markdown-rendered" }, content)), /* @__PURE__ */ h2("div", { class: "page-listing" }, showFolderCount && /* @__PURE__ */ h2("p", null, itemsUnderFolder(pages.length)), /* @__PURE__ */ h2("div", null, /* @__PURE__ */ h2(
      OttonPageList,
      {
        ...props,
        allFiles: pages,
