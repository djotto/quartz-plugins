...
function extractImageFromNodes(nodes) {
  let image;
  for (const child of nodes) {
    if (child.type === "text") {
      if (child.value.trim()) return void 0;
      continue;
    }
...
function extractImageFromParagraph(node) {
  return isParagraph(node) ? extractImageFromNodes(node.children) : void 0;
}
function embeddedImages(nodes) {
  const firstImage = nodes.findIndex(
    (child) => child.type === "element" && isFigureMedia(child)
  );
  if (firstImage < 0) return void 0;
  ...
  for (const child of nodes.slice(firstImage)) {
    ...
    if (child.type !== "element" || !isFigureMedia(child)) return void 0;
    images.push(child);
  }
  return { caption, images };
}
function isFigureMedia(node) {
  return node.tagName === "img" || node.tagName === "object";
}
function ensureLazyLoading(image) {
...
function ensureClassName(element, className) {
  ...
  const classes = Array.isArray(existing) ? existing.filter((value) => typeof value === "string") : typeof existing === "string" ? existing.split(/\s+/).filter(Boolean) : [];
  if (!classes.includes(className)) classes.push(className);
  element.properties.className = classes;
}
function labelFromAlt(properties, fallback) {
...
function isStandaloneMediaParagraph(node) {
  return node.tagName === "p" && node.children.length > 0 && node.children.every(
    (child) => child.type === "text" && child.value.trim().length === 0 || child.type === "element" && isFigureMedia(child)
  ) && node.children.some(
    (child) => child.type === "element" && isFigureMedia(child)
  );
}
function wrapStandardFigures(tree) {
  const walk = (parent, insideFigure) => {
    for (let index = 0; index < parent.children.length; index += 1) {
      ...
      if (!insideFigure && isStandaloneMediaParagraph(child)) {
        const media = child.children.filter(
          (candidate) => candidate.type === "element" && isFigureMedia(candidate)
        );
        const wrappers = media.map(
          (image) => figureWrapper({
            type: "element",
            tagName: "figure",
            properties: {},
...
var dimensionsCache = /* @__PURE__ */ new Map();
var contentFilesCache = /* @__PURE__ */ new Map();
async function contentFiles(contentRoot) {
...
function hasNumericDimension(value) {
...
async function addMediaMetadata(tree, ctx, file) {
  ...
  visit(tree, "element", (node) => {
    ...
    const source = typeof node.properties[sourceProperty] === "string" ? String(node.properties[sourceProperty]) : "";
    if (!source || /^(?:[a-z][a-z\d+.-]*:)?\/\//i.test(source) || source.startsWith("data:"))
      return;
    ...
    if (node.tagName === "object") {
      const relativeAssetPath = path.relative(path.resolve(contentRoot), assetPath).replaceAll(path.sep, "/");
      node.properties.data = `/${slugifyFilePath(relativeAssetPath)}`;
      return;
    }
    if (hasNumericDimension(node.properties.width) || hasNumericDimension(node.properties.height))
      return;
    tasks.push(
...
function resolveImageAsset(contentRoot, noteDirectory, source, allFiles) {
  ...
  const entries = allFiles.map((absolutePath) => {
    ...
    return {
      absolutePath,
      relative,
      slug: slugifyFilePath(relative),
      basenameSlug: slugifyFilePath(path.posix.basename(relative))
    };
  });
  const findSlug = (candidate) => {
    const slug = slugifyFilePath(path.posix.normalize(candidate));
  ...
  const basenameSlug = slugifyFilePath(path.posix.basename(cleaned));
  ...
  return matches.slice().sort((left, right) => {
    const leftDepth = path.relative(noteRoot, left.absolutePath).split(path.sep).length;
    const rightDepth = path.relative(noteRoot, right.absolutePath).split(path.sep).length;
    return leftDepth - rightDepth;
  })[0].absolutePath;
}
function buildCaptionFigure(image, caption) {
...
function removeLegacyCaptionMarker(nodes) {
  for (const child of nodes) {
    if (child.type === "text") {
...
function transformLegacyCaptions(tree) {
  const walk = (parent, insideFigure) => {
    for (let index = 0; index < parent.children.length; index += 1) {
      const child = parent.children[index];
      if (child.type !== "element") continue;
      if (!insideFigure && isStandaloneMediaParagraph(child)) {
        ...
        while (parent.children[captionIndex]?.type === "text" && !parent.children[captionIndex].value.trim()) {
          captionIndex += 1;
        }
        const caption = parent.children[captionIndex];
        const image = extractImageFromParagraph(child);
        if (image && caption?.type === "element" && isParagraph(caption) && removeLegacyCaptionMarker(caption.children)) {
...
function buildCompareFigure(beforeImage, afterImage, caption) {
...
function directiveKind(directive, source) {
...
function requireImage(nodes, kind, source) {
...
function requireParagraphImage(node, kind, source) {
...
function transformOttonFigures(tree, file) {
  ...
  visit(
    ...
    "element",
    (node, index, parent) => {
      if (!parent || index === void 0 || !isParagraph(node)) return;
      const paragraphNodes = node.children;
      const embeddedDirective = parseDirective(paragraphNodes);
      if (embeddedDirective) {
        const kind2 = directiveKind(embeddedDirective, source);
        const embedded = embeddedImages(paragraphNodes);
        if (embedded) {
          const expected2 = kind2 === "caption" ? 1 : 2;
          if (embedded.images.length !== expected2)
            failCount(kind2, expected2, embedded.images.length);
          if (!hasRenderableCaption(embedded.caption)) {
            throw new Error(
              `[OttonFigures] ${source}: {{${kind2}}} requires caption text after the directive`
            );
          }
          const replacement2 = kind2 === "caption" ? buildCaptionFigure(embedded.images[0], embedded.caption) : buildCompareFigure(
            embedded.images[0],
            embedded.images[1],
            embedded.caption
          );
      ...
      if (hasBreaks) {
        ...
        const parsed2 = embeddedDirective ?? parseDirective(lines[firstNonBlank]);
        if (!parsed2) return;
        const kind2 = directiveKind(parsed2, source);
        const expected2 = kind2 === "caption" ? 1 : 2;
        const imageLines = lines.slice(firstNonBlank + 1).filter((line) => !isBlankLine(line));
        if (imageLines.length !== expected2)
          failCount(kind2, expected2, imageLines.length);
        if (!hasRenderableCaption(lines[firstNonBlank])) {
          throw new Error(
            `[OttonFigures] ${source}: {{${kind2}}} requires caption text after the directive`
          );
        }
        const images2 = imageLines.map(
          (line) => requireImage(line, kind2, source)
        );
        const replacement2 = kind2 === "caption" ? buildCaptionFigure(images2[0], lines[firstNonBlank]) : buildCompareFigure(images2[0], images2[1], lines[firstNonBlank]);
        parent.children.splice(index, 1, replacement2);
        return;
      }
      ...
      const images = consumed.map(
        (paragraph) => requireParagraphImage(paragraph, kind, source)
      );
      const replacement = kind === "caption" ? buildCaptionFigure(images[0], paragraphNodes) : buildCompareFigure(images[0], images[1], paragraphNodes);
      parent.children.splice(index, expected + 1, replacement);
    }
  );
}
var sliderScript = String.raw`
(() => {
...
var DirectiveFigure = OttonFigures;
var index_default = OttonFigures;
export {
  DirectiveFigure,
  OttonFigures,
  beforeAfterClasses,
